...
Ext.define('mark.MarkModel', {		//ziji
...
var store = new Ext.data.Store({
			...
			listeners : {
				write : function(store, operation) {
					...
					Ext.example.msg(name, Ext.String.format("{0} mark: {1}",
									verb, record.getId()));
				}
			},
			// 每页显示的记录行数
...
markGrid.getSelectionModel().on('selectionchange',
		function(selModel, selections) {
			markGrid.down('#delete').setDisabled(selections.length === 0);
		});
...
var clearForm = function() {
	...
	markForm.getForm().reset();
}

var queryForm = function() {
	Ext.Msg.alert('查询', '将开始执行查询！');
}
var markForm = new Ext.form.FormPanel({
			title : '信息查询',
			width : 200,
			height : 200,
			frame : true,
