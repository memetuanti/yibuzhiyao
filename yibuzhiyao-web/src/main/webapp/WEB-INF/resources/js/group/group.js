...
var store = new Ext.data.Store({
			...
			listeners : {
				write : function(store, operation) {
					...
					Ext.example.msg(name, Ext.String.format("{0} group: {1}",
									verb, record.getId()));
				}
			},
			// 每页显示的记录行数
...
groupGrid.getSelectionModel().on('selectionchange',
		function(selModel, selections) {
			groupGrid.down('#delete').setDisabled(selections.length === 0);
		});
